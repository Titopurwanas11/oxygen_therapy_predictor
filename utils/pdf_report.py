...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        footer_text_center = "Research Prototype"
...
def generate_pdf_report(
    ...
    header_left = [
        ...
        Spacer(1, 2),
        Paragraph("Prediction Report", ParagraphStyle("HeaderPR", parent=style_body_bold, textColor=HexColor("#1e40af"))),
    ]

    header_right = [
        Paragraph(f"<b>Date:</b> {date_str}", style_body),
        Paragraph(f"<b>Time:</b> {time_str}", style_body),
        Paragraph("<i>Generated Automatically</i>", ParagraphStyle("GenAuto", parent=style_body, textColor=HexColor("#64748b"))),
    ]
    ...
    patient_rows = [
        [Paragraph("<b>Clinical Feature</b>", style_table_header), Paragraph("<b>Patient Value</b>", style_table_header)],
        [Paragraph("Patient Age", style_body), Paragraph(age_str, style_body_bold)],
        [Paragraph("Gender", style_body), Paragraph(str(gender_val), style_body_bold)],
        [Paragraph("Weight", style_body), Paragraph(weight_str, style_body_bold)],
        [Paragraph("Height", style_body), Paragraph(height_str, style_body_bold)],
        [Paragraph("Temperature", style_body), Paragraph(temp_str, style_body_bold)],
        [Paragraph("Respiratory Rate", style_body), Paragraph(rr_str, style_body_bold)],
        [Paragraph("Heart Rate", style_body), Paragraph(hr_str, style_body_bold)],
        [Paragraph("Oxygen Saturation (SaO2)", style_body), Paragraph(sao2_str, style_body_bold)],
        [Paragraph("Diagnosis Date", style_body), Paragraph(date_str, style_body_bold)]
    ]
    ...
    pred_outcome_text = "Need Oxygen Therapy" if is_yes else "No Oxygen Therapy Required"
    ...
    prediction_card_content = [
        [
            Paragraph("<b>MODEL PREDICTION SUMMARY</b>", style_subtitle),
            ""
        ],
        [
        ...
        [
            Paragraph("Prediction Probability:", style_body),
        ...
        [
            Paragraph("Risk Level:", style_body),
    ...
    story.append(Paragraph("🔮 Model Prediction", style_section_heading))
    ...
    # ─────────────────────────────────────────────────────────────────────────
    summary_box_content = [
        [Paragraph("🩺 Clinical Summary", style_section_heading)],
    ...
    shap_rows = [[
        ...
        Paragraph("Patient Value", style_table_header),
        Paragraph("SHAP Contribution", style_table_header),
        Paragraph("Clinical Effect", style_table_header)
    ]]
    ...
    story.append(Paragraph("📊 Top Contributing Features", style_section_heading))
    story.append(shap_table)
    story.append(Spacer(1, 8))

    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 5: AI INTERPRETABILITY
    # ─────────────────────────────────────────────────────────────────────────
    interpretability_text = (
        "<b>AI Interpretability Note:</b><br/>"
        "• Positive SHAP values increase the likelihood of oxygen therapy.<br/>"
        "• Negative SHAP values decrease the likelihood.<br/>"
        "• Feature importance is calculated using SHAP TreeExplainer."
    )
    ...
    if recommendation:
        ...
        rec_table_content = [
            [
                Paragraph("<b>CLINICAL RECOMMENDATION CDSS SUMMARY</b>", style_subtitle),
                ""
            ],
            [
                Paragraph("Priority Level:", style_body),
                Paragraph(f"<b>{prio} Priority</b>", style_prio)
            ],
            [
                Paragraph("Recommendation Source:", style_body),
                Paragraph("Rule-Based CDSS Recommendation Engine", style_body)
            ]
        ]
        ...
        rec_bullets_content = [
            [Paragraph("<b>Clinical Actions:</b>", style_body_bold), Paragraph("<b>Monitoring Guidelines:</b>", style_body_bold)],
        ...
        rec_disclaimer_box = Table([[Paragraph(f"<b>CDSS Disclaimer & Guidelines:</b><br/>{rec_disclaimer_text}", style_body)]], colWidths=[504])
        ...
        story.append(Paragraph("🩺 Clinical Recommendation Engine", style_section_heading))
    ...
    # ─────────────────────────────────────────────────────────────────────────
    model_info_rows = [
        [Paragraph("<b>Parameter / Metric</b>", style_table_header), Paragraph("<b>Value / Specification</b>", style_table_header)],
        [Paragraph("Algorithm", style_body), Paragraph("Random Forest Classifier", style_body_bold)],
        [Paragraph("Number of Trees", style_body), Paragraph("800", style_body_bold)],
        [Paragraph("Accuracy", style_body), Paragraph("89.44%", style_body_bold)],
        [Paragraph("F1 Macro", style_body), Paragraph("86.20%", style_body_bold)],
        [Paragraph("ROC-AUC", style_body), Paragraph("90.93%", style_body_bold)],
        [Paragraph("Dataset", style_body), Paragraph("801 Pediatric Patients", style_body_bold)],
        [Paragraph("Features", style_body), Paragraph("44 Clinical Variables", style_body_bold)]
    ]
    ...
    model_section = [
        Paragraph("⚙️ Model Information", style_section_heading),
        model_table,
        Spacer(1, 10)
    ]
    ...
    disclaimer_text = (
        "<b>DISCLAIMER</b><br/>"
        "This report is generated automatically by the OxyPredict Clinical Decision Support System. "
        "Predictions are intended to assist healthcare professionals and must not replace clinical judgment, "
        "patient examination, laboratory findings, or institutional treatment guidelines. "
        "Final medical decisions remain the responsibility of licensed healthcare professionals."
    )
